...
export default function AllAttendersSheetTab({
  ...
  const filteredLogs = useMemo(() => {
    ...
    return flattenedLogs.filter(log => {
      ...
      if (dateFrom || dateTo) {
        const targetDate = parseTimestamp(log.lastCalledAt || log.createdAt);
        if (!targetDate || isNaN(targetDate.getTime())) return false;
        const targetDateStr = getLocalDateStr(targetDate);
        if (dateFrom && targetDateStr < dateFrom) return false;
        if (dateTo && targetDateStr > dateTo) return false;
      }
              ...
              {paginatedLogs.map((log, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!hiddenColumns.includes("Attender") && (
                      <td className="py-2.5 px-4 border-r border-slate-100 align-top bg-indigo-50/20">
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-semibold bg-indigo-100 text-indigo-800 border border-indigo-200">
                          <Users size={11} />
                          {renderVal(log.attenderName, "Unassigned")}
                        </span>
                      </td>
                    )}
                    ...
                    {!hiddenColumns.includes("Phone") && (
                      <td className="py-2.5 px-4 border-r border-slate-100 font-mono text-xs text-slate-700 align-top whitespace-nowrap">
                        {renderVal(log.Phone || log.Mobile || getContactPhone(log), "—")}
                      </td>
                    )}

                    {!hiddenColumns.includes("Mobile") && (
                      <td className="py-2.5 px-4 border-r border-slate-100 font-mono text-xs text-slate-600 align-top whitespace-nowrap">
                        {renderVal(log.Mobile || getFieldWithFallback(log, "Mobile"), "—")}
                      </td>
                    )}
                    ...
                    {!hiddenColumns.includes("Khoji") && (
                      <td className="py-2.5 px-4 border-r border-slate-100 align-top">
                        {log.Khoji ? (
                          <span className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase ${isKhojiAffirmative(log.Khoji) ? "bg-amber-100 text-amber-800 border border-amber-200" : "bg-slate-100 text-slate-600"}`}>
                            {renderVal(log.Khoji)}
                          </span>
                        ) : "—"}
                      </td>
                    ...
                    {!hiddenColumns.includes("Tags") && (
                      <td className="py-2.5 px-4 border-r border-slate-100 align-top min-w-[140px]">
                        {(() => {
                    ...
                    {!hiddenColumns.includes("Source") && (
                      <td className="py-2.5 px-4 border-r border-slate-100 text-slate-700 align-top">
                        {renderVal(log.Source || log.source, "—")}
                      </td>
                    )}

                    {!hiddenColumns.includes("Called For") && (
                      <td className="py-2.5 px-4 border-r border-slate-100 text-slate-700 align-top">
                        {renderVal(log["Called For"] || log.calledFor, "—")}
                      </td>
                    )}

                    {!hiddenColumns.includes("Sub Program") && (
                      <td className="py-2.5 px-4 border-r border-slate-100 text-slate-600 align-top">
                        {renderVal(log["Sub Program"] || log.subProgram, "—")}
                      </td>
                    )}

                    {!hiddenColumns.includes("Type") && (
                      <td className="py-2.5 px-4 border-r border-slate-100 align-top">
                        <span className={`text-[10px] font-semibold uppercase px-2 py-0.5 rounded ${log.callType === "incoming" ? "bg-emerald-100 text-emerald-800" : "bg-blue-100 text-blue-800"}`}>
                          {renderVal(log.callType || "outgoing")}
                        </span>
                      </td>
                    )}
