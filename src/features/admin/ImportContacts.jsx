...
export default function ImportContacts({ programs, onImportComplete }) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-5">
      {/* Title Header */}
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-lg text-white shadow-2xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-lg font-bold tracking-tight">Lead Sync & Distribution 📂</h2>
          <p className="text-xs text-slate-400 mt-0.5">Fetch leads directly from CRM tags, group into sub-programs, and assign to attenders.</p>
        </div>

        {/* Live CRM Status Health Badge */}
        <div className="bg-slate-800/80 p-3 rounded-md border border-slate-700/60 flex items-center gap-3 w-full md:w-auto">
          <div className="w-8 h-8 bg-slate-700/50 rounded flex items-center justify-center">
            <Database size={16} className="text-indigo-400" />
          </div>
          <div>
            <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Connection Health</p>
            {crmStatus === "checking" && (
              <div className="flex items-center gap-1 text-xs font-medium animate-pulse text-slate-300">
            ...
            {crmStatus === "failed" && (
              <p className="text-xs font-semibold text-rose-400 flex items-center gap-1" title={crmError}>
                <AlertCircle size={13} /> Failed Offline
              </p>
            )}
          </div>
        </div>
      ...
      {crmStatus === "failed" && (
        <div className="p-3 bg-rose-50 text-rose-800 rounded-md text-xs font-medium border border-rose-200 flex items-start gap-2 max-w-6xl mx-auto shadow-2xs">
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-2xs space-y-4">
        ...
        <div className="space-y-1.5 relative">
          <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
            <Tag size={12} className="text-indigo-600" /> CRM Search Tag / Query
          </label>
          <div className="flex flex-col md:flex-row gap-2.5 relative">
            <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full h-9 pl-9 pr-3 bg-white border border-slate-200 rounded-md text-xs font-medium text-slate-800 focus:ring-1 focus:ring-indigo-500 outline-none"
              />

              {/* Autocomplete Suggestion Dropdown */}
              {showSuggestions && suggestions.length > 0 && (
                <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-slate-200 rounded-md shadow-lg z-50 max-h-52 overflow-y-auto border-t-0">
                  <div className="p-2 bg-slate-50 text-[10px] font-semibold text-slate-400 uppercase tracking-wider border-b border-slate-100">Recent Queries & Programs</div>
                  {suggestions.map(s => (
                    <button
                      key={s}
                      onMouseDown={() => {
                      ...
                      className="w-full text-left px-3 py-2 hover:bg-slate-50 text-xs font-medium text-slate-700 border-b border-slate-100 last:border-0 transition-colors"
                    >
                      {s}
                    </button>
                  ))}
            ...
            <div className="md:w-56 shrink-0">
              {isCrmFetching ? (
                <button
                  ...
                  className="w-full h-9 bg-rose-600 hover:bg-rose-700 text-white font-medium rounded-md shadow-2xs transition-colors flex items-center justify-center gap-1.5 text-xs cursor-pointer"
                >
                  <AlertCircle size={14} />
                  Stop Fetching Leads
                </button>
                ...
                <button
                  ...
                  className="w-full h-9 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-md shadow-2xs transition-colors flex items-center justify-center gap-1.5 text-xs disabled:bg-slate-100 disabled:text-slate-400 disabled:shadow-none cursor-pointer"
                >
                  <Sparkles size={14} />
          ...
          {crmFetchProgress && (
            <div className="mt-3 space-y-1.5 bg-indigo-50/60 p-3 rounded-md border border-indigo-100 max-w-xl">
              <div className="flex justify-between text-xs font-semibold text-indigo-900">
                ...
                <span className="animate-spin"><RefreshCw size={12} className="text-indigo-600" /></span>
              </div>
